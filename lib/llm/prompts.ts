...
절대 규칙
1. 종목코드를 쓰지 않는다. 회사는 **이름으로만** 쓴다.
2. companies 에는 **국내(KOSPI·KOSDAQ) 상장사만** 쓴다.
   비상장사, 외국 기업(TSMC·엔비디아·애플), 그룹명("삼성그룹")은 쓰지 않는다.
   확실하지 않으면 쓰지 마라 — 빈 배열이 틀린 이름보다 낫다.
   네가 쓴 이름은 상장사 사전과 대조되고, 없으면 버려진다.
3. 기사에 없는 사실을 근거로 쓰지 않는다. 추론은 경제 논리로만 전개한다.
...
- companies: 이 단계에 실제로 해당하는 **국내 상장사**를 최대 5개. 각 회사마다
  왜 그 회사인지를 **사업 구조로** 쓴다. 현직 펀드매니저가 읽고 납득할 수준이어야 한다.

  좋은 예) name="포스코퓨처엠", reason="이차전지 양극재·음극재를 생산해 리튬 등
           핵심광물을 대량 조달한다. 원료 조달선 다변화가 곧 원가에 반영된다."
  나쁜 예) name="포스코퓨처엠", reason="이차전지 관련 기업이다."
           → 무엇이 어떻게 실적에 닿는지가 없다. 이런 문장은 쓰지 마라.

  ⚠️ 규모나 인지도로 고르지 마라. **그 사건이 매출·원가에 실제로 닿는 회사**를 골라라.
  대형주라서 넣는 것이 가장 흔한 실수다. 떠오르지 않으면 빈 배열로 둔다.
${INJECTION_GUARD}`;
